...
public interface ISubareaDAO extends JpaRepository<Subarea, Long> {
    ...
    @Query("SELECT s FROM Subarea s WHERE UPPER(s.nombre) LIKE '%' + UPPER(:nombre) + '%' AND  (-1=:estadoId OR s.estado.tablaAuxiliarDetalleId.id = :estadoId) ")
    public Page<Subarea> findAllPageAndSort(String nombre, Integer estadoId, Pageable pageable);
